...
//	uint16_t getIpUsrLen()        { return getIpTotLen() - getIpHdrLen(); }
	...
	uint32_t getTcpSeq()          { return ntoh32(ETH_HDR_LEN + getIpHdrLen() + 4); }
	...
	NetdumpIP getIP6(uint16_t idx) { return NetdumpIP((const uint8_t*)&data[idx],false);
	                               };


	NetdumpIP sourceIP()          { NetdumpIP ip;
									if (isIPv4())
									{
